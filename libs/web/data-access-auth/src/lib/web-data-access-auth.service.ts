import { User, WebDataAccessService } from '@angular-graphql/web/data-access'
...
export class WebDataAccessAuthService {
  ...
  login({ username, password }) {
    return this.data.sdk.login({ data: { username, password } }).pipe(
  ...
  register({ email, password, name, username }) {
    return this.data.sdk.register({ data: { email, password, name, username } }).pipe(
